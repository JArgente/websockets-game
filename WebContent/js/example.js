...
Weapon.EightWay = function (game) {
    ...
    for (var i = 0; i < 96; i++)
    {
        this.add(new Bullet(game, 'enemy', enemy.strength), true);
    }
...
PhaserGame.prototype = {
		...
		preload: function() {
			this.load.atlasJSONHash('boss', 'img/boss.png', 'img/boss.json');
			this.load.atlasJSONHash('rangedEnemy', 'img/seta2.png', 'img/seta2.json');
			...
			this.load.audio('music', 'sounds/mundo1.mp3');
		...
		//	this.enemyBulletsPool.createMultiple(100, 'enemyBullet');
		    ...
		    this.rangedEnemyPool.createMultiple(50, rangedEnemy.skin, 'seta/0000');
		...
		enemyHit: function (bullet, foe) {
		    ...
		    explosionSound.play();
		    if(foe.type==1){
		    	explosion = this.add.sprite(foe.body.x+foe.width/2, foe.body.y+foe.height/2, 'explosion1', 1);
				 anim = explosion.animations.add('burst');
				 anim.play(15,false, true);
		    	boss.life=boss.life-bullet.damage;
		    	if(boss.life<=0){
		    		this.barSprite.kill();
		    		var animDead=bossSprite.animations.play('die');
		    		this.playersPool.forEachAlive(function(playerIngame) {
		    		 ...
		    		 bossSprite.body.checkCollision.down=false;
		    		 bossSprite.body.checkCollision.up=false;
		    		 bossSprite.body.checkCollision.right=false;
		    		 bossSprite.body.checkCollision.left=false;
		    		 
		    		finalBoss=false;
		    		animDead.onComplete.add(function(){
		    			musicSound.stop();
						batalla.stop();
					    winSound.play();
		    	...
		    	explosion = this.add.sprite(foe.body.x+foe.width/2, foe.body.y, 'explosion1', 1);
				 anim = explosion.animations.add('burst');
				 anim.play(15,false, true);
		...
		//	for(i=0;i<=5;i++){
				...
				enemyAsBullet=this.add.sprite(boss.sprite.body.x, boss.sprite.body.y+boss.sprite.height/2, "enemy");
				enemyAsBullet.life=enemy.life;
				enemyAsBullet.type=0;
				this.enemyPool.add(enemyAsBullet);
				game.physics.arcade.moveToObject(enemyAsBullet,target,120);
				boss.sprite.lastAttack=this.game.time.now;
		//	}
		},
		majorBossAtack: function(sprite, animation){
			if(animCounter>=3){
				sprite.animations.play('idle');
				blasterSound.play();
				boss.weapons.fire(sprite);
				sprite.lastAttack=this.game.time.now;
			}else{
				animCounter++;
			}
		},
			...
			this.physics.arcade.overlap(this.bossBulletsPool, this.playersPool, this.bossBulletHit, null, this);
			  ...
			  if(this.camera.x+window.innerWidth<worldsize-bossDominions || finalBoss){
				  ...
				  this.rangedEnemyPool.forEachAlive(function(rangedEnemyIngame) {
					  if(this.game.time.now > rangedEnemyIngame.lastShot+rangedEnemyIngame.fireRate){
						  var animAttack=rangedEnemyIngame.animations.play('attack');
						/*  animAttack.onComplete.add(function(){
							 var enemyBullet = globalGame.add.sprite(rangedEnemyIngame.body.x+rangedEnemyIngame.width/2, rangedEnemyIngame.body.y+15, 'enemyBullet'); //globalGame.enemyBulletsPool.getFirstExists(false);
							//  if(enemyBullet!=null){
								  globalGame.enemyBulletsPool.add(enemyBullet);
								  //enemyBullet.reset(rangedEnemyIngame.body.x+rangedEnemyIngame.width/2, rangedEnemyIngame.body.y+15);
				  ...
				  if(this.camera.x+2*window.innerWidth<worldsize-initBossDominions){
					  if(startGame && !finalBoss && this.game.time.now > this.nextEnemy && this.enemyPool.countDead() > 0){
						  ...
						  enemySprite.body.velocity.x = this.rnd.integerInRange(-80, -50);
						  
						  var random=this.game.rnd.integerInRange(4000,6000);
					  ...
					  if(startGame && !finalBoss && this.game.time.now > this.nextRangedEnemy && this.rangedEnemyPool.countDead() > 0){
						  if((window.innerWidth+this.camera.x)-lastRangedEnemyX>=500){
							  ...
							  rangedEnemyLocal.fireRate=this.rnd.integerInRange(6000, 8000);
							  rangedEnemyLocal.lastShot=this.game.time.now;
							  
							  var animAttack=rangedEnemyLocal.animations.play('attack');
							  animAttack.onComplete.add(function(){
								  blasterSound.play();
								 // alert("bala");
								  var enemyBullet = globalGame.add.sprite(rangedEnemyLocal.body.x+rangedEnemyLocal.width/2, rangedEnemyLocal.body.y+15,'enemyBullet');
								//  if(enemyBullet!=null){
								//	  enemyBullet.reset(rangedEnemyLocal.body.x+rangedEnemyLocal.width/2, rangedEnemyLocal.body.y+15);
								  globalGame.enemyBulletsPool.add(enemyBullet);
						  ...
						  var randomRanged=this.game.rnd.integerInRange(6000,7000);
			  ...
			  if(finalBoss && !animatedFinalBoss){
				  if(this.game.time.now>boss.sprite.lastAttack+boss.sprite.fireRate){
					  var attackType=this.game.rnd.integerInRange(1,4);
					  if(attackType==1){
						  animCounter=0;
						  boss.sprite.lastAttack=this.game.time.now+10000;
						  anim = bossSprite.animations.play('attack');
						  anim.onLoop.add(this.majorBossAtack, this);
					  }else{
				  ...
				  if(finalBoss==false){
					 ...
					 // var image = this.cache.getImage(boss.skin);
					  //capguy = game.add.sprite(0, 180, 'cityscene', 'capguy/walk/0001');
					    bossSprite=this.add.sprite(worldsize-bossDominions-300, window.innerHeight-350, boss.skin, 'Untitled-2/0000');
					    bossSprite.animations.add('idle', Phaser.Animation.generateFrameNames('Untitled-2/', 0, 25, '', 4), 10, true, false);
					    bossSprite.animations.add('attack', Phaser.Animation.generateFrameNames('Untitled-2/', 0, 25, '', 4), 30, true, false);
					    bossSprite.animations.add('die', Phaser.Animation.generateFrameNames('Untitled-2b/', 0, 53, '', 4), 20, false, false);
					    bossSprite.animations.play('idle');
					    bossSprite.anchor.set(0.5);
					    bossSprite.scale.setTo(0.75, 0.75);
					     
					    bossSprite.type=1;
					    bossSprite.fireRate=3000;
			  ...
			  if(!yaperdido && this.game.time.now>= yaperdidoFinal+5000){
				  window.location='finJuego1.html' 
			  }
			  if(frames==0){
				  window.location='finJuego1.html'
			  }
