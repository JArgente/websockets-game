var websocket;
var avionSel=1;
var INACTIVE = 0;
var ACTIVE = 1;
var SECONDS_INACTIVE = 0.1;

function loadSprite(src, callback) {
    var sprite = new Image();
    sprite.onload = callback;
    sprite.src = src;
    return sprite;
}

$(document).ready(function(){
	$("#tablaSelChar").on("touchstart",function(e){
	    e.preventDefault();
	});
	$("#winner").on("touchstart",function(e){
	    e.preventDefault();
	});
	$("#pistas").on("touchstart",function(e){
	    e.preventDefault();
	});
	$("#tablaPrincipal").on("touchstart",function(e){
    	e.preventDefault();
    });
    $("#looser").on("touchstart",function(e){
    	e.preventDefault();
    });
    
    $("#disparar").on("touchstart",function(e){
    	websocket.send("fire");
    });
    
    $("#disparar").on("touchend",function(e){
    	websocket.send("stopFire");
    });
    
    $("#avion1").on("touchstart",function(e){
    	if(avionSel!=1){
    		websocket.send("transform1");
    		avionSel=1;
    	}
    });
    $("#avion2").on("touchstart",function(e){
    	if(avionSel!=2){
    		websocket.send("transform2");
    		avionSel=2;
    	}
    });
    $("#avion3").on("touchstart",function(e){
    	if(avionSel!=3){
    		websocket.send("transform3");
    		avionSel=3;
    	}
    });
    
        var joystickView = new JoystickView(250, function(callbackView){
            $("#joystickContent").append(callbackView.render().el);
            setTimeout(function(){
                callbackView.renderSprite();
            }, 0);
        });
 });

function startGamePad(tipo){
	 $("#tablaSelChar").css("display","none");
	 $("#tablaPrincipal").css("visibility","visible");
	 $("#avion1").attr("src","img/"+tipo+"-1.png");
	 $("#avion2").attr("src","img/"+tipo+"-2.png");
	 $("#avion3").attr("src","img/"+tipo+"-3.png");
	// $("#tablaPrincipal").css("height",window.innerHeight);
}

JoystickView = Backbone.View.extend({
    events: {
        "touchstart": "startControl",
        "touchmove": "move",
        "touchend": "endControl",
    },
    initialize: function(squareSize, finishedLoadCallback){
    	websocket = new WebSocket("ws://192.168.1.44:8080/QuanticTravel/actions");
    	websocket.onmessage = function(message){
    		prefix=message.data.substring(0, 5);
    		if(prefix==="block"){
    			var blocked=parseInt(message.data.charAt(message.data.length-1));
    			if(blocked==0){
    				$("#vikingo").css("opacity","0.5");
    				$("#vikingo").unbind( "touchstart" );
    			}else if(blocked==1){
    				$("#cruzado").css("opacity","0.5");
    				$("#cruzado").unbind( "touchstart" );
    			}else{
    				$("#ninja").css("opacity","0.5");
    				$("#ninja").unbind( "touchstart" );
    			}
    		}else if(prefix==="initt"){
    			charDispo= message.data.substring(5, message.data.length).split("-");
    			if(charDispo!=null && charDispo.length>0){
    				for(i=0;i<charDispo.length;i++){
    						if(charDispo[i]==0){
    							$("#vikingo").css("opacity","1");
    							$("#vikingo").on("touchstart",function(e){
    						    	websocket.send("char0");
    						    	startGamePad("plane");
    						    });
    							$("#vikingo").on("click",function(e){
    						    	websocket.send("char0");
    						    	startGamePad("plane");
    						    });
    						}if(charDispo[i]==1){
    							$("#cruzado").css("opacity","1");
    							$("#cruzado").on("touchstart",function(e){
    						    	websocket.send("char1");
    						    	startGamePad("heli");
    						    });
    						}if(charDispo[i]==2){
    							$("#ninja").css("opacity","1");
    							$("#ninja").on("touchstart",function(e){
    						    	websocket.send("char2");
    						    	startGamePad("zep");
    						    });
    						}
    				}
    			}
    		}else if(prefix==="plane"){
    			 $("#avion"+message.data.charAt(message.data.length-1)).css("display","block");
    		}else if(prefix==="weapo"){
    			$("#disparar").attr("src","img/arma"+message.data.charAt(message.data.length-1)+".png");
    		}else if(prefix==="lifee"){
    			suffix=message.data.substring(5, message.data.length);
    			$("#vida").animate({width: suffix+"%"},500, function(){
    				if(suffix>70){
    					$("#vida").css("background-color","green");
    				}else if(suffix<=70 && suffix>40){
    					$("#vida").css("background-color","yellow");
    				}else if(suffix<=40 && suffix>10){
    					$("#vida").css("background-color","orange");
    				}else if(suffix<=10){
    					$("#vida").css("background-color","red");
    				}
    			});
    		}else if(prefix==="killl" || prefix==="gameo"){
    			$("#tablaPrincipal").css("display","none");
    			$("#looser").css("display","block");
    			$("#looser").animate({top: 0},500,function(){});
    		}else if(prefix==="winne"){
    			window.location='sawMovil.html'
    			$("#tablaPrincipal").css("display","none");
    			$("#winner").css("display","block");
    			$("#winner").animate({top: 0},500,function(){});
    		}else if(prefix==="pista"){
    			$("#winner").css("display","none");
    			$("#pistas").css("display","block");
    		}
    	}
    	this.ws = websocket;
        this.squareSize = squareSize;
        this.template = _.template($("#joystick-view").html());
        this.state = INACTIVE;
        this.x = 0;
        this.y = 0;
        this.canvas = null;
        this.context = null;
        this.radius = (this.squareSize / 2) * 0.5;

        this.finishedLoadCallback = finishedLoadCallback;

        this.joyStickLoaded = false;
        this.backgroundLoaded = false;
        this.lastTouch = new Date().getTime();
        self = this;
        this.sprite = loadSprite("img/button.png", function(){
            self.joyStickLoaded = true;
            self._tryCallback();
        });
        this.background = loadSprite("img/canvas.png", function(){
            self.backgroundLoaded = true;
            self._tryCallback();
        });
    },
   
    _tryCallback: function(){
        if(this.backgroundLoaded && this.joyStickLoaded){
            var self = this;
            this.finishedLoadCallback(self);
        }
    },
    startControl: function(evt){
    	//evt.preventDefault();
        this.state = ACTIVE;
    },
    endControl: function(evt){
        this.state = INACTIVE;
        this.x = 0;
        this.y = 0;
        this.renderSprite();
        this.ws.send(0+"y"+0);
    },
    move: function(evt){
        if(this.state == INACTIVE){
            return;
        }
        this.lastTouch = new Date().getTime();

        var x, y;


        if(evt.originalEvent && evt.originalEvent.touches){
            evt.preventDefault();
            var left = 0;
            var fromTop = 0;
            elem = $(this.canvas)[0];
            while(elem) {
                left = left + parseInt(elem.offsetLeft);
                fromTop = fromTop + parseInt(elem.offsetTop);
                elem = elem.offsetParent;
            }
            x = evt.originalEvent.touches[0].clientX - left;
            y = evt.originalEvent.touches[0].clientY - fromTop;
        } else {
            x = evt.offsetX;
            y = evt.offsetY;
        }
        
        this._mutateToCartesian(x, y);
        this._triggerChange();
    },
    _triggerChange: function(){
        var xPercent = this.x / this.radius;
        var yPercent = this.y / this.radius;
        if(Math.abs(xPercent) > 1.0){
            xPercent /= Math.abs(xPercent);
        }
        if(Math.abs(yPercent) > 1.0){
            yPercent /= Math.abs(yPercent);
        }
       
        this.trigger("horizontalMove", xPercent);
        this.trigger("verticalMove", yPercent);
        this.ws.send(xPercent+"y"+yPercent);
    },
    _mutateToCartesian: function(x, y){
        x -= (this.squareSize) / 2;
        y *= -1;
        y += (this.squareSize) / 2;
        if(isNaN(y)){
            y = this.squareSize / 2;
        }

        this.x = x;
        this.y = y;
        if(this._valuesExceedRadius(this.x, this.y)){
            this._traceNewValues();
        }
        
        this.renderSprite();
    },
    _retractToMiddle: function(){
        var percentLoss = 0.1;
        var toKeep = 1.0 - percentLoss;

        var xSign = 1;
        var ySign = 1;

        if(this.x != 0){
            xSign = this.x / Math.abs(this.x);
        }
        if(this.y != 0) {
            ySign = this.y / Math.abs(this.y);
        }

        this.x = Math.floor(toKeep * Math.abs(this.x)) * xSign;
        this.y = Math.floor(toKeep * Math.abs(this.y)) * ySign;
    },
    _traceNewValues: function(){
        var slope = this.y / this.x;
        var xIncr = 1;
        if(this.x < 0){
            xIncr = -1;
        }
        for(var x=0; x<this.squareSize / 2; x+=xIncr){
            var y = x * slope;
            if(this._valuesExceedRadius(x, y)){
                break;
            }
        }
        this.x = x;
        this.y = y;
    },
    _cartesianToCanvas: function(x, y){
        var newX = x + this.squareSize / 2;
        var newY = y - (this.squareSize / 2);
        newY = newY * -1;
        return {
            x: newX,
            y: newY
        }
    },
    _valuesExceedRadius: function(x, y){
        if(x === 0){
            return y > this.radius;
        }
        return Math.pow(x, 2) + Math.pow(y, 2) > Math.pow(this.radius, 2);
    },
    renderSprite: function(){
        var originalWidth = 89;
        var originalHeight = 89;

        var spriteWidth = 100;
        var spriteHeight =100;
        var pixelsLeft = 0; //ofset for sprite on img
        var pixelsTop = 0; //offset for sprite on img
        var coords = this._cartesianToCanvas(this.x, this.y);
        if(this.context == null){
            return;
        }
        // hack dunno why I need the 2x
        this.context.clearRect(0, 0, this.squareSize * 2, this.squareSize);

        var backImageSize = 300;
        this.context.drawImage(this.background,
            0,
            0,
            backImageSize,
            backImageSize,
            0,
            0,
            this.squareSize,
            this.squareSize
        )
        this.context.drawImage(this.sprite,
            pixelsLeft,
            pixelsTop,
            originalWidth,
            originalHeight,
            coords.x - spriteWidth / 2,
            coords.y - spriteHeight / 2,
            spriteWidth,
            spriteHeight
        );
    },
    render: function(){
        var renderData = {
            squareSize: this.squareSize
        };
        this.$el.html(this.template(renderData));
        this.canvas = this.$('#joystickCanvas')[0];
        this.context = this.canvas.getContext('2d');
        this.renderSprite();
        return this;
    }
});
