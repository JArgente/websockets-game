...
Weapon.EightWay = function (game) {
    ...
    for (var i = 0; i < 96; i++)
    {
        this.add(new Bullet(game, 'balaCanon2', enemy.strength), true);
    }
...
PhaserGame.prototype = {
		...
		preload: function() {
			this.load.atlasJSONHash('boss', 'img/juego3/torreon.png', 'img/juego3/torreon.json');
			this.load.image('canon', 'img/juego3/ballesta.png');
			this.load.image('fondo', 'img/juego3/fondo3.png');
			...
			this.load.image('enemy', 'img/juego3/griffin2.png');
			//this.load.image('rangedEnemy', 'img/rangedEnemy.png');
			
			//this.load.image('boss', 'img/boss.jpg');
			
			this.load.image('balaCanon', 'img/juego3/flecha.png');
			this.load.image('balaCanon2', 'img/juego2/bala2.png');
			for(i=1; i<= 6; i++){
			...
			this.load.audio('music', 'sounds/mundo3.mp3');
		},
		...
		enemyHit: function (bullet, foe) {
		    ...
		    if(foe.type==1){
		    	...
		    	if(boss.life<=0 && finalBoss){
		    		...
		    		finalBoss=false;
		    		this.barSprite.kill();
		    		var animDie=foe.animations.play('destroy');
		...
		update: function(){
			...
			this.physics.arcade.overlap(this.bossBulletsPool, this.playersPool, this.bossBulletHit, null, this);
			  ...
			  if(this.camera.x+window.innerWidth<worldsize-bossDominions || finalBoss){
				  ...
				  this.rangedEnemyPool.forEachAlive(function(rangedEnemyIngame) {
					  if(this.game.time.now > rangedEnemyIngame.lastShot+rangedEnemyIngame.fireRate){
						  ...
						  blasterSound.play();
						  enemyBullet.body.velocity.y=-120;
						  enemyBullet.body.velocity.x=-120;
						  rangedEnemyIngame.lastShot=this.game.time.now;
					  }
				  },this);
				  this.enemyPool.forEachAlive(function(enemyIngame) {
					  if(enemyIngame.body.y<=60 && enemyIngame.body.velocity.y<0){
						  enemyIngame.body.velocity.y=50;
					  }else if(enemyIngame.body.y>=window.innerHeight-210 && enemyIngame.body.velocity.y>0){
						  enemyIngame.body.velocity.y=-50;
					  }
				  },this);
				  if(this.camera.x+2*window.innerWidth<worldsize-initBossDominions){
					  if(startGame && !finalBoss && this.game.time.now > this.nextEnemy && this.enemyPool.countDead() > 0){
						  ...
						  enemySprite.body.velocity.x = this.rnd.integerInRange(-140, -100);
						  var dirY=this.rnd.integerInRange(0,1);
						  if(dirY==0)
							  enemySprite.body.velocity.y= -50;
						  else
							  enemySprite.body.velocity.y= 50;
						  enemySprite.anchor.setTo(0.5,0.8);
						  //enemySprite.body.angularVelocity=5;
						  var random=this.game.rnd.integerInRange(1700,3000);
					  ...
					  if(startGame && !finalBoss && this.game.time.now > this.nextRangedEnemy && this.rangedEnemyPool.countDead() > 0){
						  if((window.innerWidth+this.camera.x)-lastRangedEnemyX>=500){
							  ...
							  rangedEnemyLocal.fireRate=this.rnd.integerInRange(3000, 5000);
						  ...
						  var randomRanged=this.game.rnd.integerInRange(6000,8000);
			  ...
			  if(this.camera.x+window.innerWidth>=worldsize-initBossDominions && this.camera.x+window.innerWidth<worldsize-bossDominions && !this.gameoverAnim){
				  if(finalBoss==false){
					 ...
					 // var image = this.cache.getImage(boss.skin);
					    bossSprite=this.add.sprite(worldsize-bossDominions-300, window.innerHeight-370, boss.skin, 'torrreonDestruido/0000');
					    bossSprite.animations.add('destroy', Phaser.Animation.generateFrameNames('torrreonDestruido/', 0, 37, '', 4), 10, false, false);
			  ...
			  if(!yaperdido && this.game.time.now>= yaperdidoFinal+5000){
				  window.location='finJuego3.html' 
			  }
			  if(frames==0){
				  window.location='finJuego3.html'
			  }
